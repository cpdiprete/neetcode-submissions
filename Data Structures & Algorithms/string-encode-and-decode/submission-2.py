class Solution:
    ...
    def decode(self, s: str) -> List[str]:
        ...
        while (i < len(s)):
            ...
            rList.append(piece)
            print(piece)
            # print(f"looking for start: {s[(i + curLengthEncoding + 1)::]}")
            ...
            i = start # reset i 
        return rList
        start = s[0:s.find('&')] # this is the first word's length
        startInt = int(start)
        charSlot = len(start) + startInt # this is where my first word will end
        # rlist.append(s[0:start + ])
        # now that I have the first lenght, I can just go about 
        print(start)
        return ["x", "sh"]
