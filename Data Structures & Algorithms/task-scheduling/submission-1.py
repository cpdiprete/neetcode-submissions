...
class CalvinsHeap:
    ...
    def add_tasks(self, counter):
        for taskchar, count in counter.items():
            heapq.heappush(self.heap, (count * -1, taskchar)) # so we order them based on task priority
    ...
    def run_task(self):
        ...
        elif len(self.heap) == 0 and len(self.bench) == 0:
            # bc priority was 0 and there are no more task to run
            return False
        # now we can pull any items into play from the bench that have sat long enough
        self.clear_bench()
        return True
